            ...
            if int_time == np.inf:
                ...
                for lb_subtraction in test_lb_subtractions:
                    ...
                    while not converged and lb_adjustment < max_adjustement:
                        ...
                        # dMag_init = tmp[np.argmin(np.abs(tmp2))]
                        # dMag_init = (dMag_lb + singularity_dMag) / 2.0
                        # bounds = (dMag_lb, singularity_dMag - 1e-6)
                        ...
                        # status check
                        message = (
                            dMag_min_res["message"][0]
                            if isinstance(dMag_min_res["message"], np.ndarray)
                            else dMag_min_res["message"]
                        )
                        success = (
                            dMag_min_res["success"][0]
                            if isinstance(dMag_min_res["success"], np.ndarray)
                            else dMag_min_res["success"]
                        )
